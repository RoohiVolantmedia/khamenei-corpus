...
def _get_db_path() -> Path:
    ...
    if url:
        with st.spinner("در حال بارگذاری پایگاه داده... (یک‌بار در هر راه‌اندازی)"):
            urllib.request.urlretrieve(url, str(tmp))
        _DB_CACHE = tmp
        return _DB_CACHE

    raise FileNotFoundError(
        "database.db پیدا نشد.\n"
        "متغیر SUPABASE_DB_URL را در Streamlit Secrets تنظیم کنید."
    )
